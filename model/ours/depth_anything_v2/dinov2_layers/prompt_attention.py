...
class PromptAttention(Attention):
    ...
    def forward(self, x_ori: Tensor, prompt=None) -> Tensor:
        ...
        if prompt is not None:
            ...
            #
            # # 保持旧版完全相同的加性掩码语义：
            # # 有效Key位置为0，无效Key位置为-inf。
            # # 只是不再复制成[B, H, N, N]，而是由SDPA自动广播。
            # prompt_mask = prompt_mask.view(B, 1, 1, N)
            #
            # prompt_attn_mask = torch.where(
            #     prompt_mask == 0,
            #     torch.full_like(prompt_mask, float("-inf")),
            #     torch.zeros_like(prompt_mask),
            # )
            #
            # # Q、K、V、scale、dropout均保持旧版完全不变
            # prompt_output = F.scaled_dot_product_attention(
            #     prompt_q,
            #     prompt_k,
            #     prompt_v,
